class Start extends Phaser.Scene {
    ...
    create() {
       ...
       my.sprite.meteors = this.add.group({
        ...
        my.sprite.button3 = this.add.sprite(100, 580, "button");
        my.sprite.button3.setScale(4, 3);
        my.sprite.button3Text = this.add.text(my.sprite.button3.x, my.sprite.button3.y, 'Credits', { font: '32px Roboto', fill: '#FFFFFF' }).setOrigin(0.5);
        ...
        this.input.on("pointerdown", function (event) {
                ...
                } else if ((Math.abs(event.y - my.sprite.button3.y) < (my.sprite.button3.displayHeight/2)) 
                    && (Math.abs(event.x - my.sprite.button3.x) < (my.sprite.button3.displayWidth/2))) {
    ...
    update() {
        ...
        if (this.animThangRotate) {
            my.sprite.title.angle += .1;
            if (my.sprite.title.angle > 10) {
                this.animThangRotate = false;
            }
        } else {
            my.sprite.title.angle -= .1;
            if (my.sprite.title.angle < -10) {
                this.animThangRotate = true;
            }
        }

        if (this.animThangGrow) {
            my.sprite.title.scale += .01;
            if (my.sprite.title.scale > 2) {
                this.animThangGrow = false;
            }
        } else {
            my.sprite.title.scale -= .01;
            if (my.sprite.title.scale < .5) {
                this.animThangGrow = true;
            }
        }
